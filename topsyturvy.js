...
//follow_line();
function checkingturn(direction, limiter) {
    for (let i = 0; i < limiter; i = i + 1) {
        move_forward(mB, 0, 0.5, 0.1);
        move_backward(mA, 0, 0.5, 0.1);
        pause(200);
    }
}

function topsy_turvy() {
    let light_level = 0;
    let turn_count = 0;
    const dist = 0.5;
    const limit = 10;
    while (!ev3_touchSensorPressed(touchSensor)) {
        ...
        if (light_level < 10) {
            turn_count = 0;
            display('turn count reset');
            move_forward(mB, 0, dist, 0.2);
            pause(200);
            move_forward(mA, 0, dist, 0.2);
            pause(200);
            
        } else if (turn_count < limit) { //hit white
            display('turning right');
            move_forward(mB, 0, dist, 0.2);
            move_backward(mA, 0, dist, 0.2); //turn right
            pause(200);
            ...
            move_forward(mA, 0, dist, 0.2);
            move_backward(mB, 0, dist, 0.2); //turn left
            pause(200);
        }
    }
    display('ended while loop');
    pause(3000);
}
